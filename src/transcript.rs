...
pub trait FieldTranscript<F> {
    ...
    fn common_field_element(&mut self, fe: &F) -> Result<(), TranscriptError>;

    fn common_field_elements(&mut self, fes: &[F]) -> Result<(), TranscriptError> {
        fes.iter()
...
pub trait FieldTranscriptRead<F>: FieldTranscript<F> {
    fn read_field_element(&mut self) -> Result<F, TranscriptError>;

    fn read_field_elements(&mut self, n: usize) -> Result<Vec<F>, TranscriptError> {
        (0..n).map(|_| self.read_field_element()).collect()
    }
}

pub trait FieldTranscriptWrite<F>: FieldTranscript<F> {
    fn write_field_element(&mut self, fe: &F) -> Result<(), TranscriptError>;
...
pub trait Transcript<C, F>: FieldTranscript<F> {
    fn common_commitment(&mut self, comm: &C) -> Result<(), TranscriptError>;

    fn common_commitments(&mut self, comms: &[C]) -> Result<(), TranscriptError> {
        comms
            .iter()
...
pub trait TranscriptRead<C, F>: Transcript<C, F> + FieldTranscriptRead<F> {
    fn read_commitment(&mut self) -> Result<C, TranscriptError>;

    fn read_commitments(&mut self, n: usize) -> Result<Vec<C>, TranscriptError> {
        (0..n).map(|_| self.read_commitment()).collect()
    }
}

pub trait TranscriptWrite<C, F>: Transcript<C, F> + FieldTranscriptWrite<F> {
    fn write_commitment(&mut self, comm: &C) -> Result<(), TranscriptError>;
    ...
    ($($curve:ty),*$(,)?) => {
        $(
            impl<H: Hash, S> Transcript<$curve, <$curve as CurveAffine>::ScalarExt> for FiatShamirTranscript<H, S> {
                fn common_commitment(&mut self, comm: &$curve) -> Result<(), TranscriptError> {
                    let coordinates =
            ...
            {
                fn read_commitment(&mut self) -> Result<$curve, TranscriptError> {
            ...
            {
                fn write_commitment(&mut self, ec_point: &$curve) -> Result<(), TranscriptError> {
                    self.common_commitment(ec_point)?;
...
impl<F: PrimeField, S> Transcript<Output<Keccak256>, F> for Keccak256Transcript<S> {
    fn common_commitment(&mut self, comm: &Output<Keccak256>) -> Result<(), TranscriptError> {
...
impl<F: PrimeField, R: io::Read> TranscriptRead<Output<Keccak256>, F> for Keccak256Transcript<R> {
    fn read_commitment(&mut self) -> Result<Output<Keccak256>, TranscriptError> {
...
impl<F: PrimeField, W: io::Write> TranscriptWrite<Output<Keccak256>, F> for Keccak256Transcript<W> {
    fn write_commitment(&mut self, hash: &Output<Keccak256>) -> Result<(), TranscriptError> {
        self.stream
            .write_all(hash)
            .map_err(|err| TranscriptError)?;
        Ok(())
